...
export const CAMPAIGN_RULES: {
  name: string
  mustMatch: string[][]
  // 이 키워드가 caption/해시태그에 있으면 이 룰은 매칭 제외 (이번/이전 캐릭터가 겹치는 경우 구분용)
  exclude?: string[]
}[] = [
  {
  ...
  },
  {
    ...
    // 이번 콜라보 (갸루 신짱) - 먼저 체크. colorgram/컬러그램 브랜드명 필수
    mustMatch: [
  ...
  },
  {
    ...
    // 이전 콜라보 - ギャル/갸루 언급 시 이번 콜라보로 간주. colorgram/컬러그램 브랜드명 필수
    mustMatch: [
      ["colorgram", "컬러그램", "カラーグラム"],
      ["クレヨンしんちゃんコラボ", "クレヨンリップ", "クレヨンしんちゃん", "しんちゃん", "짱구", "クレヨン"],
    ],
